...
class CausalCNNEncoder(torch.nn.Module):
    ...
    @param kernel_size Kernel size of the applied non-residual convolutions.
    """
    def __init__(self, in_channels, channels, depth, reduced_size,out_channels, kernel_size, global_average_pooling = True, time_series_length = None):
        super(CausalCNNEncoder, self).__init__()
        
        # If using fully-connected layer to squeeze the temporal dimension, the length of the time series should be known.
        assert global_average_pooling==True or (global_average_pooling==False and time_series_length is not None)
